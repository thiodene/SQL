...
if(isset($_POST['username'])&&isset($_POST['mysql'])&&isset($_POST['db'])&&isset($_POST['username']))
{
        ...
        //$r1=mysqli_fetch_array($result, MYSQLI_NUM) ;
        ...
        //$r3=mysqli_fetch_array($result3, MYSQLI_NUM) ;
        $r3= mysqli_num_rows($result) ;

        $count=$r3-$r1;
        if($count>0)
        $display_block = "Success";
        $display_block = "<b> total $count records have been added to the table $table </b> ";
}
else
{
        $display_block = "Mysql Server address/Host name ,Username , Database name ,Table name , File name are the Mandatory Fields";
}
...
<form class="form-horizontal"action="load_csv.php" method="post">
